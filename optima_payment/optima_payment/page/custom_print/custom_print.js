frappe.pages["custom_print"].on_page_load = function (wrapper) {
		...
		if (!frappe.route_options || !frappe.route_options.frm) {
			frappe.model.with_doc(doctype, docname, () => {
				...
				frappe.model.with_doctype(doctype, () => {
					...
					print_view.show(frm);
					print_view.fetch_bank_print_formats(frm);
				});
			});
		} else {
...
frappe.ui.form.PrintView = class {
	...
	async fetch_bank_print_formats(frm) {
		...
		try {
			const frm_mode_of_payment = this.frm.doc.mode_of_payment;
			const mode_of_payment = await frappe.db.get_doc("Mode of Payment", frm_mode_of_payment);
			console.log(mode_of_payment.type);
			this.type = mode_of_payment.type;
			if (this.type !== "Cheque") {
				this.type = "";
				return;
			};
			const bank_name = this.frm.doc.bank_name; // Get the bank name from the form
	...
	setup_toolbar() {
		...
		this.page.add_action_icon(
			...
			() => {
				...
				return {
					filters: this.type === "Cheque" ? { doc_type: this.frm.doctype, name: ["in", this.print_format_names] } : {
	...
	hide() {
		if (this.frm.setup_done && this.frm.page.current_view_name === "custom_print") {
			this.frm.page.set_view(
				this.frm.page.previous_view_name === "custom_print"
					? "main"
