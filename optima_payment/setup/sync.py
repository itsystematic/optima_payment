...
def adopt_custom_fields(dry_run: bool = False) -> list[Change]:
    ...
    A declared property the site holds with a value different from code may be a client edit, so
    that value is first kept as a flag-0 Property Setter, the same record Customize Form writes for
    flag-1 fields, unless a property setter already overrides it. Only then is the row reset to the
    ...
    for doctype, fields in declared.items():
        for field in fields:
            ...
            change, differences, kept_as_setters = _plan_adoption(doctype, field, row)
    ...
    _report("Optima Payment custom field adoption", changes, dry_run)
    return changes
...
def _plan_adoption(doctype: str, field: dict, row: dict) -> tuple[Change, dict[str, tuple], list[str]]:
    target = f"{doctype}.{field['fieldname']}"
    differences = _get_differences(row, field)
