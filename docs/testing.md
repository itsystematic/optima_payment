...
# A whole doctype's tests
bench --site <site> run-tests --doctype "Letter of Credit"
```
...
A few assertions depend on how a site is customized. For example, the Letter of Credit
*validity-only extend with commission* test **skips** on sites that enforce the WTS
Balance-Sheet/P&L cost-center rule (that path can't satisfy it — see
[letter-of-credit.md](letter-of-credit.md#not-yet-wired)). The CI site has no such
customization, so the test runs fully there.
